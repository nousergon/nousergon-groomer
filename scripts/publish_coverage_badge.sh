...
# editing a file. This repo carried no coverage badge at all before this
# change (§5.3 requires Tier A/B repos to carry one).
#
...
# from the same `.coverage` file the gate in pyproject.toml's
# [tool.coverage.report] fail_under exits non-zero on, and it moves on its
# own.
#
# Runs from .github/workflows/test.yml on pushes to main only, from one
# matrix leg (py3.12). The `badges` branch is seeded on the remote before the
# PR that adds this script ever opens, so the contents API is enough and no
# branch is ever created here.
set -euo pipefail
